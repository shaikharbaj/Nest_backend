  ...
  async login(payload: supplierloginDTO, res: Response) {
    try {
      ...
      const pay = {
        ...
        // user_information: checksupplierexist.,
        permission: checksupplierexist?.role?.permissions.filter((p: any) => p?.userType === "SUPPLIER").map((p) => p?.permission?.slug)
      }
      const options: { expiresIn: string } = { expiresIn: '10h' }
      const token = await this.generateToken(pay, options);

      return res.status(201).json({ success: true, message: "supplier loggedIn successfully", token, data: pay });
    } catch (error) {
